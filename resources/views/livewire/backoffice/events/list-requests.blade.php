...
<x-livewire-tables::bs5.table.cell class="text-end">
    @can('update', $row)
        <x-buttons.small-approve wire:click="$emit('showModal', 'backoffice.events.approve-request', {{ $row }})" />
        <x-buttons.small-decline wire:click="$emit('showModal', 'backoffice.events.decline-request', {{ $row }})" />
    @endcan
</x-livewire-tables::bs5.table.cell>

<x-livewire-tables::bs5.table.cell class="text-end">
    <x-button class="btn-sm btn-icon btn-icon-start btn-outline-success ms-1"
        wire:click="$emit('showModal', 'backoffice.users.user-edit', '{{ $id }}')">
    ...
    <x-button class="btn-sm btn-icon btn-icon-start btn-outline-danger ms-1"
        wire:click="$emit('showModal', 'backoffice.users.user-delete', '{{ $id }}')">
        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 20 20" fill="none"
            stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"
            class="cs-icon cs-icon-bin">
            <path
